...
Content that previously used emoji (e.g. routing callouts, result cards, timeline, CTA) is replaced with theme SVGs so icons are consistent and avoid WordPress emoji images.

- **Helper:** `ehs_get_content_icon_svg( $slug )` in `inc/content-icons.php` returns inline SVG for a slug.
- **Filter:** `the_content` and `the_excerpt` run `ehs_replace_emoji_with_svg()` to replace emoji characters and WordPress emoji `<img>` tags with `<span class="ehs-content-icon">` + SVG.
- **Slugs:** target, road, gear, clipboard, rocket, briefcase, clock, check, calendar.
- **Assets:** `assets/service-icons/` (target.svg, road.svg, gear.svg, clipboard-check.svg, rocket.svg, briefcase.svg, clock.svg, check.svg, calendar.svg).
- **CSS:** `.ehs-content-icon` and context-specific rules (e.g. `.routing-callout__icon svg`, `.result-card > div:first-child svg`) size SVGs with `1em` and `currentColor`.
